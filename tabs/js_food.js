...
const NUTRIENT_GROUPS = [
  { label: 'Macros', items: [
    ...
    { id:'sugar',      label:'Sugar (total)', unit:'g' },
    { id:'addedSugar', label:'Added sugar',   unit:'g' },
  ]},
  ...
  { label: 'Minerals', items: [
    ...
    { id:'iron',       label:'Iron (total)',   unit:'mg'  },
    { id:'ironHeme',   label:'Iron (heme)',    unit:'mg'  },
    ...
    { id:'selenium',   label:'Selenium',       unit:'mcg' },
...
function injectFoodStyles() {
  ...
  s.textContent = `
    ...
    .nutrient-bal-note-v2{ font-size:9.5px; font-weight:400; color:var(--text4,#a8967c); margin-top:1px; line-height:1.3; max-width:220px; }
...
// Personal reference profile — used only to size the "too little / OK /
// too much" ranges below. Update these numbers if they change.
// Disclosed conditions: thalassemia minor, arthritis, Behçet's disease
// with uveitis. These aren't diagnosed or managed by this app — just used
// to shift a couple of reference ranges and add context notes below.
// Always defer to your actual doctors' guidance over this.
const USER_PROFILE = { sex:'male', ageYears:24, weightKg:88, heightCm:175,
  conditions:['thalassemia minor','arthritis',"Behçet's disease with uveitis"] };

// Measured BMR (actual test result) — overrides the Mifflin-St Jeor
// estimate since a real measurement beats a formula guess. TDEE uses a
// sedentary/extremely-inactive multiplier (1.2) on top of it. This is
// still a planning number, not a prescription.
const BMR = 1500;
const TDEE = Math.round(BMR * 1.2);
const PROTEIN_TARGET_G = Math.round(USER_PROFILE.weightKg * 1.7); // ~active-maintenance range

// Reference ranges, sized to the profile above (adult male). min = aim for
// at least this; max = try to stay under this; both = a comfortable
// range. Rough general guidance, not medical advice. `note` = a short
// context line shown under that nutrient's label, tied to a disclosed
// condition above — only added where it's directly relevant.
const NUTRIENT_TARGETS = {
  calories:{ min:Math.round(TDEE*0.85), max:Math.round(TDEE*1.15) },
  protein:{ min:PROTEIN_TARGET_G, max:Math.round(PROTEIN_TARGET_G*1.8) },
  carbs:{ min:180, max:350 }, fat:{ min:55, max:100 }, satFat:{ max:22 },
  fiber:{ min:32 }, sugar:{ max:70 },
  addedSugar:{ max:36, note:'Added sugar (not natural fruit/dairy sugar) — the one that actually correlates with inflammation.' },
  vitA:{ min:900 }, vitC:{ min:90, note:'Antioxidant, relevant for arthritis/Behçet\'s inflammation.' },
  vitD:{ min:20, note:'Watch alongside calcium if ever on steroids for a flare (bone health).' },
  vitE:{ min:15, note:'Antioxidant, relevant for inflammatory conditions.' },
  vitK:{ min:120 },
  b1:{ min:1.2 }, b2:{ min:1.3 }, b3:{ min:16 }, b6:{ min:1.7 }, b12:{ min:2.4 }, folate:{ min:400 },
  calcium:{ min:1000, note:'Watch alongside vitamin D if ever on steroids for a flare (bone health).' },
  // Iron ceiling, not floor: thalassemia minor + male (no menstrual iron
  // loss) means overload is the actual risk here, not deficiency. A "Low"
  // reading is usually fine — ask your hematologist before ever
  // supplementing iron.
  iron:{ min:6, max:20, note:'Thalassemia minor — overload risk matters more than a low reading here. Don\'t supplement without your hematologist.' },
  ironHeme:{ note:'Portion of total iron that\'s heme (meat/fish) — absorbs far more efficiently than plant iron.' },
  magnesium:{ min:420 }, potassium:{ min:3400 },
  sodium:{ max:2300, note:'Watch closer if ever on steroids for a flare (fluid retention).' },
  zinc:{ min:11 }, phosphorus:{ min:700 },
  selenium:{ min:55, note:'Antioxidant, supports immune regulation — relevant for autoimmune/inflammatory conditions.' },
  iodine:{ min:150 },
  cholesterol:{ max:300 },
  omega3:{ min:1600, note:'Anti-inflammatory — relevant for arthritis/Behçet\'s. Fatty fish, walnuts, flax.' },
  choline:{ min:550 },
  caffeine:{ max:400 }, alcohol:{ max:20 },
  collagen:{}, probiotic:{},
};

// Turns a value into Low / OK / High against the ranges above.
function nutrientStatus(id, value) {
  const t = NUTRIENT_TARGETS[id];
  if (!t || (t.min == null && t.max == null)) return { label:'tracked', color:'var(--text4)', pct:null };
  if (t.min != null && value < t.min) {
    const pct = Math.round((value / t.min) * 100);
    return { label: pct < 60 ? 'Very low' : 'Low', color:'var(--f-warn)', pct };
  }
  if (t.max != null && value > t.max) {
    const pct = Math.round((value / t.max) * 100);
    return { label: pct > 150 ? 'Very high' : 'High', color:'var(--f-bad)', pct };
  }
  const pct = t.min != null ? Math.round((value / t.min) * 100) : Math.round((value / t.max) * 100);
  return { label:'OK', color:'var(--f-ok)', pct };
}

// One nutrient row: label (+ context note if this nutrient has one), value,
// status bar + Low/OK/High badge. Shared by the stats "Nutrient Balance"
// card and each Full History day.
function nutrientStatusRowHtml(labelText, unit, value, targetId, valSuffix) {
  const s = nutrientStatus(targetId, value);
  const barPct = s.pct != null ? Math.min(100, s.pct) : 0;
  const note = (NUTRIENT_TARGETS[targetId] || {}).note;
  return `<div class="nutrient-bal-row-v2">
    <div class="nutrient-bal-label-v2">${labelText}${note ? `<div class="nutrient-bal-note-v2">${note}</div>` : ''}</div>
    <div class="nutrient-bal-val-v2">${value} ${unit}${valSuffix ? '<span>'+valSuffix+'</span>' : ''}</div>
    ${s.pct != null ? `<div class="nutrient-bal-bar-v2"><div class="nutrient-bal-fill-v2" style="width:${barPct}%;background:${s.color}"></div></div>` : '<div></div>'}
    <span class="nutrient-bal-status-v2" style="color:${s.color}">${s.label}${s.pct!=null?' · '+s.pct+'%':''}</span>
  </div>`;
}
...
function computeNutrientAverages(fullLog) {
  const loggedDays = (fullLog||[]).filter(e => (e.calories||0) > 0 || (e.meals||[]).some(m => (m.items||[]).length));
  const sums = {}, dayCounts = {};
  loggedDays.forEach(e => {
    const dayTotals = {};
    (e.meals||[]).forEach(m => (m.items||[]).forEach(it => {
      const n = computeItemNutrients(it);
      Object.keys(n).forEach(k => { dayTotals[k] = (dayTotals[k]||0) + n[k]; });
    }));
    Object.keys(dayTotals).forEach(id => {
      sums[id] = (sums[id]||0) + dayTotals[id];
...
function renderNutrientBalanceCard(fullLog) {
  ...
  const groupsHtml = NUTRIENT_GROUPS.map(g => {
    ...
    const rows = items.map(n => {
      ...
      return nutrientStatusRowHtml(n.label, n.unit, a.avg, n.id, ' /day avg · '+a.daysTracked+'d');
    }).join('');
  ...
  return `<div class="fm-card" style="margin-bottom:20px;">
    ...
    <div style="font-size:11px;color:var(--text4);font-style:italic;margin-bottom:10px;">Reference ranges sized to your profile (${USER_PROFILE.ageYears}${USER_PROFILE.sex==='male'?'M':'F'}, ${USER_PROFILE.weightKg}kg, ${USER_PROFILE.heightCm}cm), with iron flipped to a ceiling not a floor (thalassemia minor) and a few notes tied to arthritis/Behçet's — a rough nudge, not medical advice. Only nutrients you've logged data for are shown.</div>
    ${groupsHtml}
  </div>`;
}

// For chart display only: fills unlogged (0-value) days with the average
// of the nearest logged day before and after, instead of dropping to zero
// (which would misleadingly look like "ate nothing" rather than "didn't
// log"). Runs of several unlogged days in a row all get the same filled
// average. Days at the very start/end with no anchor on one side just
// borrow the nearest available value. Marks each filled point so the
// chart can render it visually distinct from real logged data.
function interpolateZeroGaps(rows) {
  ...
  while (i < n) {
    ...
    let j = i;
    while (j < n && vals[j] === 0) j++; // j = first index after this gap (or n)
    const prevVal = i > 0 ? filled[i-1] : null;
    const nextVal = j < n ? vals[j] : null;
    let fillVal;
    if (prevVal != null && nextVal != null) fillVal = Math.round((prevVal + nextVal) / 2);
    else if (prevVal != null) fillVal = prevVal;
    else if (nextVal != null) fillVal = nextVal;
    else fillVal = 0;
    for (let k = i; k < j; k++) filled[k] = fillVal;
    i = j;
  }
  return rows.map((r, idx) => ({ ...r, val: filled[idx], interpolated: vals[idx] === 0 && filled[idx] > 0 }));
}
...
function renderFoodStats(data) {
  ...
  <div class="fcharts-col-v2">
    <div class="fchart-card-v2">
      ...
      <div class="fchart-sub-v2">Last 30 days · red dashes = ${CAL_TARGET} kcal target · faded bars = no log that day, estimated from neighbors</div>
      <div class="mchart-wrap" style="height:240px"><canvas id="fCalChart"></canvas></div>
    </div>
    <div class="fchart-card-v2">
      <div class="fchart-title-v2">💪 Daily Protein</div>
      <div class="fchart-sub-v2">Last 30 days · red dashes = ${PROT_TARGET}g target · faded bars = no log that day, estimated from neighbors</div>
  ...
  setTimeout(() => {
    ...
    const calRows = interpolateZeroGaps(cals);
    const calLabels = calRows.map(d=>d.jalali||d.date);
    const calVals = calRows.map(d=>d.val);
    const calColors = calRows.map(d=>d.interpolated?'rgba(212,130,26,.28)':'rgba(212,130,26,.65)');
    const c1 = document.getElementById('fCalChart');
    if (c1) foodHistChart.cal = new Chart(c1, { type:'bar', data:{ labels:calLabels, datasets:[
      {label:'Calories',data:calVals,backgroundColor:calColors,borderRadius:4,maxBarThickness:26},
    ...
    const protRows = interpolateZeroGaps(prots);
    const protLabels = protRows.map(d=>d.jalali||d.date);
    const protVals = protRows.map(d=>d.val);
    const protColors = protRows.map(d=>d.interpolated?'rgba(58,122,90,.28)':'rgba(58,122,90,.65)');
    ...
    if (c2) foodHistChart.prot = new Chart(c2, { type:'bar', data:{ labels:protLabels, datasets:[
      {label:'Protein',data:protVals,backgroundColor:protColors,borderRadius:4,maxBarThickness:26},
...
function renderFoodFullHistory(fullLog) {
  ...
  const rows = sorted.map((entry, idx) => {
    ...
    return `
      ...
      <div class="fhist-details-v2">${detailsHtml}${entry.notes ? '<div class="fhist-meal-line-v2" style="margin-top:6px;"><b>Notes:</b> '+entry.notes+'</div>' : ''}
        ${renderDayNutrientsFor(entry)}
      </div>`;
  }).join('');
...
// Every macro/nutrient number for one logged day, each flagged Low / OK /
// High against the personal reference ranges — computed live from current
// ingredient data so it's accurate for old entries too.
function renderDayNutrientsFor(entry) {
  const totals = {};
  (entry.meals||[]).forEach(m => (m.items||[]).forEach(it => {
    const n = computeItemNutrients(it);
    Object.keys(n).forEach(k => { totals[k] = Math.round(((totals[k]||0) + n[k]) * 100) / 100; });
  }));
  const cal = entry.calories || 0, prot = entry.protein || 0;
  const primaryHtml = (cal || prot) ? `
    ${cal ? nutrientStatusRowHtml('Calories','kcal',cal,'calories') : ''}
    ${prot ? nutrientStatusRowHtml('Protein','g',prot,'protein') : ''}` : '';
  ...
  const groupsHtml = NUTRIENT_GROUPS.map(g => {
    ...
    return `<div class="nutrient-group-label-v2">${g.label}</div>
      ${rows.map(n => nutrientStatusRowHtml(n.label, n.unit, totals[n.id], n.id)).join('')}`;
  }).join('');
  return `<div class="nutrient-panel-v2" style="display:block;margin-top:8px;">
    <div style="font-size:10px;text-transform:uppercase;letter-spacing:.05em;color:var(--text4);font-weight:700;margin-bottom:4px;">Full nutrient breakdown <span style="text-transform:none;font-weight:400;">— vs. your reference (${USER_PROFILE.ageYears}${USER_PROFILE.sex==='male'?'M':'F'}, ${USER_PROFILE.weightKg}kg, ${USER_PROFILE.heightCm}cm)</span></div>
    ${primaryHtml}
    ${hasAny ? groupsHtml : '<div style="color:var(--text4);font-size:11.5px;font-style:italic;margin-top:4px;">No extended nutrient data for items logged this day.</div>'}
  </div>`;
}
...
// Computes a logged item's nutrients from the CURRENT ingredient database
// (id lookup × qty) rather than trusting whatever was frozen on the item
// when it was added. This is what makes nutrient data show up for food
// logged before this feature existed, and keeps it correct if you edit an
// ingredient's nutrients later. Falls back to whatever's stored on the
// item itself only if the ingredient no longer exists (e.g. deleted).
function computeItemNutrients(item) {
  const ingr = F.ingredients.find(i => i.id === item.ingredient_id);
  if (ingr && ingr.nutrients && Object.keys(ingr.nutrients).length) {
    const out = {};
    ALL_NUTRIENT_IDS.forEach(id => {
      const v = ingr.nutrients[id];
      if (v) out[id] = Math.round(v * (item.qty||0) * 100) / 100;
    });
    return out;
  }
...
function openMealItemInfoCard(mealId, idx) {
  ...
  document.getElementById('ingrInfoCard').innerHTML = buildInfoCardHTML(
    ...
    item.calories, item.protein, computeItemNutrients(item)
  );
  el.classList.add('open');
}
...
function recalcMacrosFromItems() {
  ...
  (F.entry.meals||[]).forEach(m => (m.items||[]).forEach(it => {
    ...
    Object.keys(n).forEach(k => {
      nutrTotals[k] = Math.round(((nutrTotals[k]||0) + n[k]) * 100) / 100;
    });
  }));
