...
// Chart builder
// `interpolate:true` marks metrics where 0/null means "not logged" rather than a
// real zero — gaps in these get filled with the average of the nearest logged
// day before and after, so a missed day doesn't look like a cliff in the chart.
const METRICS={
  // ── Core score ──
  score:{label:'Score (pts)',fn:d=>d.score,color:'#e85d3a',group:'General',interpolate:true},
  // ── Body & mood ──
  mood:{label:'Mood',fn:d=>d.mood,color:'#3a7a5a',group:'Body & Mind',interpolate:true},
  pain:{label:'Pain',fn:d=>d.pain,color:'#c44a6a',group:'Body & Mind',interpolate:true},
  weight:{label:'Weight (kg)',fn:d=>d.weight,color:'#7a3a6a',group:'Body & Mind',interpolate:true},
  // ── Food & nutrition ──
  ...
  // ── Sleep ──
  sleep_score:{label:'Sleep Score',fn:d=>d.sleep_score!=null?d.sleep_score:null,color:'#7a3a6a',group:'Sleep',interpolate:true},
  sleep_duration:{label:'Sleep Duration (min)',fn:d=>d.sleep_duration_mins||null,color:'#5a5a9a',group:'Sleep',interpolate:true},
  sleep_quality:{label:'Sleep Quality',fn:d=>d.sleep_quality||null,color:'#9a5a7a',group:'Sleep',interpolate:true},
  // ── Money (0 spent is a real, meaningful day — no interpolation) ──
...
// Fills 0/null/undefined gaps in a numeric series with the average of the
// nearest known value before and after the gap (or just that one neighbor if
// the gap sits at the start/end of the range).
function interpolateGaps(values){
  ...
  while(i<n){
    if(isEmpty(out[i])){
      let prevVal=null;
      for(let p=i-1;p>=0;p--){ if(!isEmpty(out[p])){prevVal=out[p];break;} }
      let j=i;
      while(j<n && isEmpty(out[j])) j++;
      const nextVal=(j<n)?out[j]:null;
      let fillVal;
      if(prevVal!=null && nextVal!=null) fillVal=(prevVal+nextVal)/2;
      else if(prevVal!=null) fillVal=prevVal;
      else if(nextVal!=null) fillVal=nextVal;
      else fillVal=null; // no logged data anywhere in the series
      for(let k=i;k<j;k++) out[k]=fillVal;
      i=j;
    } else i++;
  }
  return out;
...
function renderChartInner(cfg,skipGrid=false,isPreview=false){
  ...
  if(cfg.xField==='date'){
    ...
    const datasets=cfg.yFields.map((y,i)=>{
      const raw=statsData.map(d=>METRICS[y]?.fn(d));
      const data=METRICS[y]?.interpolate?interpolateGaps(raw):raw;
      return {
        label:METRICS[y]?.label||y, data,
